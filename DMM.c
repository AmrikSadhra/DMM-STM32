...
int main (void) {
  ...
  if (SysTick_Config(SystemCoreClock / 1000)) { /* SysTick 1 msec interrupts  */
	...
	while(1) {                                    /* Loop forever               */
		...
		if(strstr(bluetoothSwitchPacket,"<m: 4") != NULL){ bluetoothMenuPosition = FREQUENCY_RESP_STATE;
		...
		if(strstr(bluetoothSwitchPacket,"<m: 5") != NULL){ bluetoothMenuPosition = SIG_GEN_STATE;
			...
			free(genType); //Free temporary string comparison buffer
				...
				if((prevGenFrequency != genFrequency)||(prevGenAmplitude != genAmplitude)||(sigGenType!=prevSigGenType)){
					newSignal = true;
				}
		...
		if(strstr(bluetoothSwitchPacket,"<m: 8") != NULL){ bluetoothMenuPosition = DIODE_STATE;
			
			char *testType = (char *) malloc(16 * sizeof(char)); //Temporary variable with which to parse signal type to int
			...
			#ifdef DMM_DEBUG
				if(n != 1) printf("[Android Client] Failed to parse Client diode test request!\r\n"); //If not parsed 1 item from string
			#endif
		}
